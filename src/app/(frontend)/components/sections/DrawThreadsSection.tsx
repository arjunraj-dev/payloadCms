          ...
          <div className="flex flex-wrap justify-center gap-3 md:gap-4">
            {firstRow.map((tag) => (
              <Link
                key={tag.label}
                href={tag.href}
                className="inline-flex items-center justify-center rounded-full bg-[#001529] px-6 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-90 sm:py-3"
              >
                {tag.label}
              </Link>
            ))}
